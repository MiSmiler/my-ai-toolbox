...
1. **Listen and record.** As the user shares review comments, capture each one. Note the file, line, what the issue is, and any suggested fix. Write each comment to a markdown file under the `.review_sugguestion/` directory — create the directory if it doesn't exist. Use a single file per review session, named with the current date/time (e.g., `.review_sugguestion/review-2025-01-15-1430.md`).
...
Save the final summary to the session's file in `.review_sugguestion/`, then ask if they want to post the comments (e.g., via `gh pr review`) or edit further.
